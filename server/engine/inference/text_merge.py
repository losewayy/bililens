...
def merge_transcription_results(
    ...
    for i, res in enumerate(results):
        ...
        buffer_overlap_text = "".join([str(s[0]) for s in buffer_overlap_segs])
        
        # 建立字符偏移到 segment 对象的映射索引，防止多字符词元（如英文单词）导致越界
        buffer_char_to_seg = []
        for s in buffer_overlap_segs:
            for _ in range(len(str(s[0]))):
                buffer_char_to_seg.append(s)

        # 提取新片段的前部分作为匹配源
        curr_overlap_limit = overlap_s + 1.0
        curr_overlap_segs = [s for s in curr_segments if s[1] <= curr_overlap_limit]
        curr_overlap_text = "".join([str(s[0]) for s in curr_overlap_segs])

        curr_char_to_seg = []
        for s in curr_overlap_segs:
            for _ in range(len(str(s[0]))):
                curr_char_to_seg.append(s)
        ...
        if (
            match.size >= 2
            and match.a < len(buffer_char_to_seg)
            and match.b < len(curr_char_to_seg)
        ):
            # match.a 是 buffer_overlap_text 中的对齐点字符下标
            # match.b 是 curr_overlap_text 中的对齐点字符下标
            
            # a. 截断 buffer
            target_seg = buffer_char_to_seg[match.a]
            ...
            match_seg = curr_char_to_seg[match.b]
            match_idx_in_curr = -1
            for idx, s in enumerate(curr_segments):
                if s is match_seg:  # 对象级别匹配
