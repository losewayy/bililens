...
class AudioTranscriber:
    ...
    def transcribe(
        ...
        try:
            ...
            _, result.timings.total = timer(
                ...
                base_offset, temperature=temperature, top_p=top_p, top_k=top_k, on_progress=on_progress
            )
    ...
    def _process_audio(self, audio, result, language, context, verbose, segment_size, overlap, reporter, base_offset,
                       temperature=0.8, top_p=1.0, top_k=50, on_progress=None):
        audio_duration = len(audio) / self.sample_rate
        ...
        if is_multi:
            reporter.print(f"    检测到长音频（{audio_duration:.1f}s），开启分段识别模式（共 {len(segments_info)} 段）...", force=True)
        ...
        for idx, (s_s, e_s) in enumerate(segments_info):
            ...
            # 动态进度条计算
            cur_seg = idx + 1
            tot_segs = len(segments_info)
            pct = round((cur_seg / tot_segs) * 100, 1)
            t_now = time.time()
            elapsed_sec = t_now - t_batch_start
            eta_sec = round((elapsed_sec / cur_seg) * (tot_segs - cur_seg), 1) if cur_seg < tot_segs else 0.0

            bar_len = 20
            filled = int(bar_len * cur_seg // tot_segs)
            bar = "█" * filled + "░" * (bar_len - filled)
            eta_str = f"{int(eta_sec//60):02d}:{int(eta_sec%60):02d}" if eta_sec > 0 else "即将完成"
            preview_txt = (d_res.text.strip()[:16] + '..') if len(d_res.text.strip()) > 16 else d_res.text.strip()
            print(f"\r[ASR 转录进度] |{bar}| {cur_seg}/{tot_segs} ({pct:5.1f}%) | 剩余: ~{eta_str} | {preview_txt:<18}", end="", flush=True)
            ...
            if on_progress:
                try:
                    on_progress({
                        ...
                        "percent": pct,
                        "eta_sec": eta_sec,
                        "text": d_res.text.strip(),
                        "range": [round(s_s, 1), round(e_s, 1)],
                    })
                except Exception:
                    pass

        if is_multi:
            print(f"\n[ASR 进度] 全部分段识别完毕，正在执行时间戳无缝拼接...", flush=True)
